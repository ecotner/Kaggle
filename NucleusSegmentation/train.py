...
#model.convolution(f=3, s=1, n_out=512, activation='relu')
model.squeeze_convolution(f=2, s=2, n_out=512, activation='relu')
model.convolution(f=3, s=1, n_out=1024, activation='relu')
#model.convolution(f=3, s=1, n_out=1024, activation='relu')
model.stretch_transpose_convolution(f=2, s=2, n_out=512, activation='relu')
model.convolution(f=3, s=1, n_out=512, activation='relu')
#model.convolution(f=3, s=1, n_out=512, activation='relu')
...
model.add_optimizer(opt_type='momentum')
model.save_graph(MODEL_PATH/('UNet'+str(0)))

# Load the data, split into train/validation sets
print('Loading data...')
...
model.train(X_train, Y_train, X_val, Y_val, max_epochs=100, batch_size=1, learning_rate_init=3e-5, reg_param=0, learning_rate_decay_type='inverse', learning_rate_decay_parameter=10, early_stopping=True, save_path='./models/0/UNet0', reset_parameters=True, check_val_every_n_batches=20, seed=0, data_on_GPU=False)
